...
class ConnectorIT extends KafkaConnectorSpec {
    ...
    def setupSpec() {
        ...
        mq.withNetwork(KafkaConnectorSpec.network)
    ...
    def "jms-amqp sink"() {
        setup:
            ...
            def cnt = connectorContainer('jms_amqp_10_sink_v1.json', """
                kafka_topic : ${topic}
    ...
    def "jms-amqp sink with filter"() {
        setup:
            ...
            cnt.start()
        when:
        KafkaConnectorSpec.kafka.send(topic, payload1)
        KafkaConnectorSpec.kafka.send(topic, payload2)
        then:
            def records = KafkaConnectorSpec.kafka.poll(group, topic)
            records.size() == 2
    ...
    def "jms-amqp source with ValueToKey"() {
        setup:
            ...
            def cnt = connectorContainer('jms_amqp_10_source_v1.json', """
                kafka_topic: ${topic}
                kafka_bootstrap_servers: ${KafkaConnectorSpec.kafka.outsideBootstrapServers}
                jms_amqp_remote_u_r_i: 'amqp://tc-activemq:61616'
                jms_amqp_destination_name: 'cards'
        ...
        then:
            await(5, TimeUnit.SECONDS) {
                def record = KafkaConnectorSpec.kafka.poll(cnt.containerId, topic).first()
    ...
    @Ignore("temporary disabled for CPaaS")
    def "jms-amqp sink with ReplaceField with connector-rename-field-action Kamelet"() {
        setup:
            Connection connection = createConnection(mq)
            Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE)
            connection.start()
            ...
            def cnt = connectorContainer('jms_amqp_10_sink_v1.json', """
                ...
                processors:
                - to:
                    uri: 'kamelet:connector-rename-field-action?from=field3&to=field3-modified'
            """)
    ...
    def "jms-amqp source with Content based routing"() {
        ...
        KafkaConnectorSpec.kafka.createTopic("NA")
            def payloadEMEA = '''{"destination": "EMEA", "command": "CMD.EMEA.01"}'''
        KafkaConnectorSpec.kafka.createTopic("EMEA")
            def payloadLATAM = '''{"destination": "LATAM", "command": "CMD.LATAM.01"}'''
        KafkaConnectorSpec.kafka.createTopic("LATAM")
            def payloadAPAC = '''{"destination": "APAC", "command": "CMD.APAC.01"}'''
        KafkaConnectorSpec.kafka.createTopic("APAC")
        ...
        then:
            await(20, TimeUnit.SECONDS) {
                def record = KafkaConnectorSpec.kafka.poll(group, "NA").first()
                def valueNA = mapper.readTree(record.value())
                record = KafkaConnectorSpec.kafka.poll(group, "EMEA").first()
                def valueEMEA = mapper.readTree(record.value())
                record = KafkaConnectorSpec.kafka.poll(group, "LATAM").first()
                def valueLATAM = mapper.readTree(record.value())
                record = KafkaConnectorSpec.kafka.poll(group, "APAC").first()
    ...
    def "jms-amqp source with Content based routing: choice-when"() {
        setup:
            ...
            def group = UUID.randomUUID().toString()
            def payloadNA = '''{"destination": "NA", "command": "CMD.NA.01"}'''
        KafkaConnectorSpec.kafka.createTopic("TOPIC.NA")
            def payloadEMEA = '''{"destination": "EMEA", "command": "CMD.EMEA.01"}'''
        KafkaConnectorSpec.kafka.createTopic("TOPIC.EMEA")
            def payloadLATAM = '''{"destination": "LATAM", "command": "CMD.LATAM.01"}'''
        KafkaConnectorSpec.kafka.createTopic("TOPIC.LATAM")
            def payloadAPAC = '''{"destination": "APAC", "command": "CMD.APAC.01"}'''
        KafkaConnectorSpec.kafka.createTopic("TOPIC.APAC")
        ...
        then:
            await(20, TimeUnit.SECONDS) {
                def record = KafkaConnectorSpec.kafka.poll(group, "TOPIC.NA").first()
                def valueNA = mapper.readTree(record.value())
                record = KafkaConnectorSpec.kafka.poll(group, "TOPIC.EMEA").first()
                def valueEMEA = mapper.readTree(record.value())
                record = KafkaConnectorSpec.kafka.poll(group, "TOPIC.LATAM").first()
                def valueLATAM = mapper.readTree(record.value());
                record = KafkaConnectorSpec.kafka.poll(group, "TOPIC.APAC").first()
                def valueAPAC = mapper.readTree(record.value())
