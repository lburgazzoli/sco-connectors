...
@Slf4j
class AwsConnectorIT extends KafkaConnectorSpec {
    ...
    def "cw sink"() {
        setup:
            ...
            def cnt = forDefinition('aws_cloudwatch_sink_v1.yaml')
            ...
            cnt.withEnv("camel.component.aws2-cw.uriEndpointOverride", aws.endpoint)
            cnt.withEnv("camel.component.aws2-cw.overrideEndpoint", "true")
            cnt.start()
    ...
    def "s3 sink"() {
        setup:
            ...
            def cnt = forDefinition('aws_s3_sink_v1.yaml')
                .withSourceProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
    ...
    def "s3 source"() {
        setup:
            ...
            def cnt = forDefinition('aws_s3_source_v1.yaml')
                .withSinkProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
    ...
    def "sns sink"() {
        setup:
            ...
            cnt.withEnv("camel.component.aws2-sns.uriEndpointOverride", aws.endpoint)
            cnt.withEnv("camel.component.aws2-sns.overrideEndpoint", "true")
            cnt.start()
        when:
    ...
    def "sqs sink"() {
        setup:
            ...
            def cnt = forDefinition('aws_sqs_sink_v1.yaml')
                .withSourceProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
            ...
            cnt.withEnv("camel.component.aws2-sqs.uriEndpointOverride", aws.endpoint)
            cnt.withEnv("camel.component.aws2-sqs.overrideEndpoint", "true")
            cnt.start()
        when:
            kafka.send(topic, payload, ['foo': 'bar'])
        then:
    ...
    def "sqs source"() {
        setup:
            ...
            def cnt = forDefinition('aws_sqs_source_v1.yaml')
                .withSinkProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
    ...
    def "kinesis sink"() {
        setup:
            ...
            def cnt = forDefinition('aws_kinesis_sink_v1.yaml')
                .withSourceProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
            ...
            cnt.withEnv("camel.component.aws2-kinesis.uriEndpointOverride", aws.endpoint)
            cnt.withEnv("camel.component.aws2-kinesis.overrideEndpoint", "true")
            cnt.start()
        when:
            kafka.send(topic, payload, ['foo': 'bar'])
        then:
            await(10, TimeUnit.SECONDS) {
                ...
                try {
                    sharedIt = AwsConnectorSupport.getShardIterator(kinesis, topic)
                } catch (Exception e) {
                    return false
                }
    ...
    def "kinesis source"() {
        setup:
            ...
            AwsConnectorSupport.createStream(kinesis, topic)

            def cnt = forDefinition('aws_kinesis_source_v1.yaml')
                .withSinkProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
    ...
    def "ddb sink"() {
        setup:
            ...
            def cnt = forDefinition('aws_ddb_sink_v1.yaml')
                .withSourceProperties([
                        'topic': topic,
                        'bootstrapServers': kafka.outsideBootstrapServers,
                        'consumerGroup': uid(),
            ...
            cnt.withEnv("camel.component.aws2-ddb.uriEndpointOverride", aws.endpoint)
            cnt.withEnv("camel.component.aws2-ddb.overrideEndpoint", "true")
            cnt.start()
        when:
