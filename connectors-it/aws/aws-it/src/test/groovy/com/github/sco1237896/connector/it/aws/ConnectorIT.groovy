...
class ConnectorIT extends KafkaConnectorSpec {
    ...
    @Override
    def setupSpec() {
        aws = new AWSContainer(KafkaConnectorSpec.network, 'cw', 's3', 'sns', 'sqs', 'kinesis', 'dynamodb')
        aws.start()
    }
    ...
    def "cw sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_cloudwatch_sink_v1.json', [
                'kafka_topic' : topic,
            ...
            def cw = aws.cw()
        when:
        KafkaConnectorSpec.kafka.send(topic, null, ['metric-value': '2.0', 'metric-name': 'metricName1651062317458', 'metric-unit': 'Percent' ])
        then:
    ...
    def "s3 sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_s3_sink_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'kafka_consumer_group': UUID.randomUUID().toString(),
                'aws_access_key': aws.credentials.accessKeyId(),
            ...
            def s3 = aws.s3()
        when:
        KafkaConnectorSpec.kafka.send(topic, payload, ['file': fileName])
        then:
            await(10, TimeUnit.SECONDS) {
                try {
    ...
    def "s3 source"() {
        setup:
            ...
            def cnt = connectorContainer('aws_s3_source_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'aws_access_key': aws.credentials.accessKeyId(),
                'aws_secret_key': aws.credentials.secretAccessKey(),
    ...
    def "sns sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_sns_sink_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'kafka_consumer_group': UUID.randomUUID().toString(),
                'aws_access_key': aws.credentials.accessKeyId(),
            ...
            cnt.start()
        when:
        KafkaConnectorSpec.kafka.send(topic, payload, ['foo': 'bar'])
        then:
            await(10, TimeUnit.SECONDS) {
    ...
    def "sqs sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_sqs_sink_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'kafka_consumer_group': UUID.randomUUID().toString(),
                'aws_access_key': aws.credentials.accessKeyId(),
    ...
    def "sqs source"() {
        setup:
            ...
            def cnt = connectorContainer('aws_sqs_source_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'aws_access_key': aws.credentials.accessKeyId(),
                'aws_secret_key': aws.credentials.secretAccessKey(),
    ...
    def "kinesis sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_kinesis_sink_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'kafka_consumer_group': UUID.randomUUID().toString(),
                'aws_access_key': aws.credentials.accessKeyId(),
    ...
    def "kinesis source"() {
        setup:
            ...
            def cnt = connectorContainer('aws_kinesis_source_v1.json', [
                    'kafka_topic' : topic,
                    'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                    'aws_access_key': aws.credentials.accessKeyId(),
                    'aws_secret_key': aws.credentials.secretAccessKey(),
    ...
    def "ddb sink"() {
        setup:
            ...
            def cnt = connectorContainer('aws_ddb_sink_v1.json', [
                'kafka_topic' : topic,
                'kafka_bootstrap_servers': KafkaConnectorSpec.kafka.outsideBootstrapServers,
                'kafka_consumer_group': UUID.randomUUID().toString(),
                'aws_access_key': aws.credentials.accessKeyId(),
            ...
            cnt.start()
        when:
        KafkaConnectorSpec.kafka.send(topic, payload.toString(), [:])
        then:
