...
class ConnectorIT extends KafkaConnectorSpec {
    ...
    def setupSpec() {
        ...
        container.withNetwork(KafkaConnectorSpec.network)
    ...
    def "google pubsub sink"() {
        setup:
            ...
            def cnt = connectorContainer('google_pubsub_sink_v1.json', [
            ...
            cnt.start()
        when:
        KafkaConnectorSpec.kafka.send(kafkaTopic, payload)
        then:
            def records = KafkaConnectorSpec.kafka.poll(kafkaGroup, kafkaTopic)
            records.size() == 1
